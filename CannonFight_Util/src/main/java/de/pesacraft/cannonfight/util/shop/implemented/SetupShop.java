...
public class SetupShop {
	...
	public static Shop getSetupMenu(CannonFighter c) {
				...
				if (event.isPickUpAction()) {
					...
					for (Entry<Cannon, ItemStack> entry : available.entrySet()) {
						if (item.isSimilar(entry.getValue())) {
							// disable this
							String cannon = entry.getKey().getName();
							if (p.isSelected(cannon)) {
								p.deselectCannon(cannon);
								event.setNextShop(getSetupMenu(p));
							}
							else {
								p.sendMessage(Language.get("info.cannon-not-selected", true));
							}
							return;
						}
					}
	...
	private static ItemStack setupItem(CannonFighter player, Cannon cannon) {
